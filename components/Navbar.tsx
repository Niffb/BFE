  ...
  return (
    <nav 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 border-b ${
        isScrolled || isMenuOpen ? 'bg-white border-neutral-200 py-3 shadow-sm' : 'bg-white/95 border-transparent py-5 backdrop-blur-md'
      }`}
    >
      <div className="max-w-[1800px] mx-auto px-6 lg:px-12 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3 group z-50 relative" onClick={() => setIsMenuOpen(false)}>
            <div className={`w-9 h-9 flex items-center justify-center rounded-sm transition-colors duration-300 ${isScrolled || isMenuOpen ? 'bg-black text-white' : 'bg-brand-red text-white'}`}>
            ...
            <div className="flex flex-col">
                ...
                <span className="text-[9px] uppercase tracking-[0.25em] text-neutral-500 hidden sm:block mt-1 font-medium">Research Institute</span>
            </div>
        </Link>
            ...
            if (item.type === 'dropdown' && item.children) {
                ...
                return (
                    <div key={index} className="relative group">
                         <button className={`flex items-center gap-1 text-[11px] font-bold uppercase tracking-[0.1em] transition-colors duration-300 py-2 focus:outline-none ${
                             parentActive ? 'text-brand-red' : 'text-neutral-500 group-hover:text-black'
                         }`}>
                             {item.label}
                             ...
                             <div className="bg-white border border-neutral-100 shadow-xl rounded-sm p-2 flex flex-col gap-1">
                                {item.children.map((child) => (
                                    <Link 
                                        key={child.label}
                                        to={child.href}
                                        className={`block px-4 py-3 text-[11px] uppercase tracking-wider font-medium hover:bg-neutral-50 transition-colors ${
                                            isActive(child.href) ? 'text-brand-red bg-neutral-50' : 'text-neutral-600'
                                        }`}
                                    >
                                        {child.label}
                                    </Link>
                ...
                return (
                    ...
                    >
                        <span className={`text-[11px] font-bold uppercase tracking-[0.1em] transition-colors duration-300 ${
                            isActive(item.href!) ? 'text-brand-red' : 'text-neutral-500 group-hover:text-black'
                        }`}>
                            {item.label}
                        </span>
                        <span className={`absolute bottom-0 left-0 w-full h-[1px] bg-brand-red transform origin-left transition-transform duration-300 ${
      ...
      <div className={`fixed inset-0 bg-white z-40 transition-all duration-500 ease-[cubic-bezier(0.76, 0, 0.24, 1)] ${
